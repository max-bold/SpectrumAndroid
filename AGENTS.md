...
- Only log-Gaussian smoothing is supported. RTA displays 1/3-, 1/6- and 1/12-octave rows as bars. Two dense line presets use 512 logarithmically spaced points at 0.3-octave FWHM or 1024 points at 0.15-octave FWHM. Do not add a 1/128-octave row: the user meant 1/12 and rejected that dense octave grid.
...
5. For the bar presets, RTA centers follow the base-10 fractional-octave grid around 1 kHz. Even denominators have a half-step offset. Include bands whose edges intersect the measurement range, retaining the nominal 20 Hz band. FWHM is grid spacing, `log2(10^(0.3 / fraction))`, independent of the selected band. The dense line presets use logarithmically spaced endpoints within the selected band and their specified 0.3/0.15-octave FWHM. These are Gaussian-smoothed spectra, not an IEC-compliant filter bank.
...
Keep `src/model.ts`, `src/settings-controls.ts`, native `Settings.kt` and `SpectrumPlugin.kt` parsing consistent when changing settings. Validate dependent values such as hop/window size and Welch window/recording duration. `rtaFraction` retains its storage/bridge name: values 3/6/12 select octave bars, while 512/1024 select dense line presets and set `PlanKey.octaveFraction` to zero.
