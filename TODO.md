...
- [x] Verify measured spectrum slope using an external input, including the effect of de-pink correction.
- [x] Verify both generators' actual output spectra through an external measurement path. Numerical fixture tests alone do not validate the Android playback chain.
- [x] Change spectrum smoothing slider range to (0.1, 1.0) oct.
...
- [x] Investigate the issue with audio output to an external audio interface over USB OTG.
- [x] Open the settings window immediately when the settings button is pressed, without waiting for the fade-out to complete.
- [x] Verify behavior when processing takes longer than the hop interval. RTA now analyzes the most recent complete PCM window and skips missed hops; an overload regression test checks that stale windows are not queued.
